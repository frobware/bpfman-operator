...
func TestBpfmanConfigReconcileAndDelete(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(fmt.Sprintf("isOpenShift: %v", tc.isOpenShift), func(t *testing.T) {
			...
			// First reconcile will add bpfman-operator
			// finalizer to bpfman configmap.
			...
			if tc.isOpenShift {
				// Check the SCC created with the correct configuration.
				actualRestrictedSCC := &osv1.SecurityContextConstraints{}
				expectedRestrictedSCC := &osv1.SecurityContextConstraints{}
				...
				require.NoError(t, err)
				// The loaded SCC from disk will have
				// a ResourceVersion of 0; we make
				// expected and actual match as that's
			...
			err = cl.Get(ctx, types.NamespacedName{Namespace: corev1.NamespaceAll, Name: internal.BpfmanRestrictedSccName}, &osv1.SecurityContextConstraints{})
			require.True(t, errors.IsNotFound(err))
		})
	}
}

func TestBpfmanConfigReconcileAndDeleteOpenShift(t *testing.T) {
	r, bpfmanConfig, req, ctx, client := setupTestEnvironment(true)
	require.Equal(t, r.RestrictedSCC, resolveConfigPath(internal.BpfmanRestrictedSCCPath))

	// First reconcile will add bpfman-operator finalizer to
	// bpfman configmap.
	res, err := r.Reconcile(ctx, req)
	require.NoError(t, err)
	require.False(t, res.Requeue)

	// Check the ConfigMap object was updated successfully with
	// the finalizer.
	err = client.Get(ctx, types.NamespacedName{Namespace: bpfmanConfig.Namespace, Name: bpfmanConfig.Name}, bpfmanConfig)
	require.NoError(t, err)
	require.Contains(t, bpfmanConfig.GetFinalizers(), internal.BpfmanOperatorFinalizer)

	// Second reconcile will create bpfman daemonset and SCC.
	res, err = r.Reconcile(ctx, req)
	require.NoError(t, err)
	require.False(t, res.Requeue)
	...
	// Delete the bpfman configmap.
	err = client.Delete(ctx, bpfmanConfig)
	require.NoError(t, err)

	// Third reconcile will delete bpfman daemonset and SCC.
	res, err = r.Reconcile(ctx, req)
	require.NoError(t, err)
	require.False(t, res.Requeue)
...
// function will register the SecurityContextConstraints (SCC) type
// with the runtime scheme if the isOpenShift parameter is set to
// true, which is required for OpenShift-specific tests.
//
// Parameters: - isOpenShift (bool):
//
//	A flag indicating whether to set up the test environment for
//	OpenShift, which includes registering the SCC type with the
//	runtime scheme.
//
// Returns:
...
// - context.Context: The context for the reconcile function.
// - client.Client: The fake client for API interactions.
